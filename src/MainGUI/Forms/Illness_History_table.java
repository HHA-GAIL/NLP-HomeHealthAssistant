...
import MainGUI.DAO.IllnessHistoryDAO;
...
/**
 * Done! The original code of showing data through table
 *
 * @author Zhiwei Chu
 * @date 06/24/17
 */
public class Illness_History_table extends JInternalFrame {

    /*
     * in order to select rows,
     * the first colunm can be changed from ID number to check-box
     */
    private JTable table;
    private DefaultTableCellRenderer render;
    private IllnessHistoryDAO dao;
    ...
    private void insertAction() {
        ...
        String lineInfo = "<table style='font-size:12px;border:1px solid black;'>"
                ...
                + "<th style='border:1px solid black;'>Illness ID</th>"
                + "<th style='border:1px solid black;'>Patient ID</th>"
                + "<th style='border:1px solid black;'>Illness Date</th>"
                + "<th style='border:1px solid black;'>Illness</th>"
                + "<th style='border:1px solid black;'>Resolution</th>"
                + "<th style='border:1px solid black;'>Illness HRF</th>"
                + "</tr>";
        ...
        while (!addedRowNum.isEmpty()) {
            ...
            illnesshistory.setIllnessID((int) table.getValueAt(row, 0));
            illnesshistory.setPatientID((int) table.getValueAt(row, 1));
            try {
                java.util.Date date = new SimpleDateFormat("yyyy-MM-dd").parse((String) table.getValueAt(row, 2));
                java.sql.Date sqlStartDate = new Date(date.getTime());
                illnesshistory.setIllnessDate(sqlStartDate);
            } catch (ParseException ex) {
                ex.printStackTrace();
            }
            illnesshistory.setIllness((String) table.getValueAt(row, 3));
            illnesshistory.setResolution((String) table.getValueAt(row, 4));
            if ((table.getValueAt(row, 5)) == null || (table.getValueAt(row, 5)).equals(false)) {
                illnesshistory.setIllnessHRF(new Byte("0"));
            } else {
                illnesshistory.setIllnessHRF(new Byte("1"));
            }
            illnesshistory.setDeleted(new Byte("0"));
    ...
    /**
     * not only can delete the selected rows which already have the data, but
     * also can this function delete the added rows which haven't inserted into
     * database don't modify this function. Fragile !!!
     */
    private void deleteAction() {
        ...
        for (int row : rows) {
            if (addedRowNum.contains(row)) {
                ...
                illnesshistory.setDeleted(new Byte("1"));
        ...
        if (n == 0) {
            if (dao.update(illnessHistories)) {
