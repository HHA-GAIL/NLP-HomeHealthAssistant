 ...
 * @author NazonaX
 */
public class WeightDevDAO {
    private Connection conn;
    private PreparedStatement pstmt;
    private ResultSet rst;
    
    public List<Weight_Dev> getListByPatientID(int patientID) {
        try {
            conn = JDBCMySQLConnection.getConnection(JDBCMySQLConnection.TYPE.GALE);
            pstmt = conn.prepareStatement("SELECT * FROM Weight_Dev where PatientID_FK=? order by DateTime desc limit 0, 200;");
            ...
            while (rst.next()) {
                ...
                tmp.setWeight(rst.getDouble("Weight"));
                tmp.setWeightID(rst.getInt("WeightID"));
